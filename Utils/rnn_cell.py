...
class RNN_Cell(object):
    ...
    def forward(self, x, h):
        ...
        # h_prime =
        # return h_prime
        a = np.tensordot(x,self.W_ih.T,1) + self.b_ih + np.tensordot(h,self.W_hh.T,1) + self.b_hh
        z = Tanh().forward(a)
        return z
    ...
    def backward(self, delta, h, h_prev_l, h_prev_t):
        ...
        self.dW_ih += np.tensordot(dz.T,h_prev_l,1)/batch_size
        self.dW_hh += np.tensordot(dz.T,h_prev_t,1)/batch_size
        self.db_ih += np.sum(dz,axis=0)/batch_size
        self.db_hh += np.sum(dz,axis=0)/batch_size

        # 2) Compute dx, dh
        dx = np.tensordot(dz,self.W_ih,1)
        dh = np.tensordot(dz,self.W_hh,1)
