...
class RNN_Phoneme_Classifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for t in range(seq_len):
            a = self.x[:,t,:]
            for i, rnn_cell in enumerate(self.rnn):
                hidden[i,:,:] = rnn_cell.forward(a, hidden[i,:,:])
                a = hidden[i,:,:]
            self.hiddens.append(hidden.copy())
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in range(seq_len-1,-1,-1): 
            for i in range(self.num_layers-1,-1,-1):
                if i == 0:
                    dx, dh[i,:,:] = self.rnn[i].backward(dh[i,:,:],self.hiddens[t+1][i,:,:],self.x[:,t,:],self.hiddens[t][i,:,:])
                else:                               
                    dx, dh[i,:,:] = self.rnn[i].backward(dh[i,:,:],self.hiddens[t+1][i,:,:],self.hiddens[t+1][i-1,:,:],self.hiddens[t][i,:,:])
                    dh[i-1] += dx
        dh = dh/batch_size
        # dh_0 =
        # return dh_0
